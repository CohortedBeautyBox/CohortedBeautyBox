    ...
    var _onCartShippingRatesUpdate = function(rates, shipping_address) {
        ...
        if (rates.length) {
            if (rates[0].price == "0.00") {
                jQuery("#shipping-fee").html("FREE")
            } else {
            	jQuery("#shipping-fee").html(Shopify.formatMoney(rates[0].price, window.money_format));
                jQuery("#total_price").html(Shopify.formatMoney( parseFloat(rates[0].price * 100) + parseFloat(jQuery("#total_price_val").val() * 100), window.money_format));
